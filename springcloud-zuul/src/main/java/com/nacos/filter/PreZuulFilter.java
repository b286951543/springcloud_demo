 ...
 *
 * route：路由请求时被调用，route过滤器用于将外部请求转发到具体服务实例上去；
 *
 * post：在route和error过滤器之后被调用，post过滤器用于将微服务的响应信息返回到客户端，这个过程中可以对返回数据进行加工处理；
 *
 ...
 *
 * 核心逻辑：org.springframework.cloud.netflix.zuul.web.ZuulController 以及 com.netflix.zuul.http.ZuulServlet.service()
 */
@Component
